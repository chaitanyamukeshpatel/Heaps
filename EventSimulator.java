/**
 * A file to implement event simulator class.
 */
package hw6;
...
public class EventSimulator
{
    ...
    {
        int totalCustomers = eventQueue.size();
        int totalWaitTime = 0;
        int maxWaitTime = 0;
        ...
        while(!eventQueue.isEmpty())
        {
        	...
        	if(next.getType()==0)
        	{
        		...
        		{
        			busy=true;
        			Event departure = new Event(Event.DEPARTURE,(clock+next.getCustomer().getServiceTime()),next.getCustomer());        			
        		...
        		else
        		{
        			...
        			if(maxWaitQueueSize < waitLine.size())
        			{
        	...
        	else
        	{
        		if(!waitLine.isEmpty())
        		{
        			Customer next_customer = waitLine.poll();
        			int WaitTime = clock - next_customer.getArrivalTime();
        			if(WaitTime>maxWaitTime)
        			{
        				maxWaitTime = WaitTime;
        			}
        			totalWaitTime = totalWaitTime + WaitTime;
        			Event departure = new Event(Event.DEPARTURE,clock+next_customer.getServiceTime(),next_customer);
        			eventQueue.add(departure);
